...
from classy import ItolJplace, TreeProtein, TreeLeafReference, ReferenceSequence, MarkerBuild, ReferencePackage, TaxonTest, Cluster
from jplace_utils import jplace_parser
from json import loads
from fasta import Header, register_headers
import file_parsers
...
class ItolJplaceTest(unittest.TestCase):
    ...
    def test_correct_decoding(self):
        itol = create_itol()
        assert type(itol.placements[0]) is dict
        assert(itol.correct_decoding() == None)
        assert type(itol.placements[0]) is str
    ...
    def test_create_jplace_node_map(self):
        ...
        #Compare random values to expected 
        assert(itol.node_map[0][0] == '14')
        assert(itol.node_map[1][0] == '175')
...
class HeaderTest(unittest.TestCase):
    ...
    def test_get_info(self):
        ...
        assert(result_info_str == "TreeSAPP ID = ''\tPrefix = ''\nOriginal =  >2_McrA\nFormatted = ")
